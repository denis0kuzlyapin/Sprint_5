...
from locators import (HomePage, StandTesting, AuthPade, Header, ErrorAuth)
...
def test_registration(driver, random_email, random_password):
    ...
    driver.find_element(*AuthPade.EMAIL_INPUT).send_keys(random_email)
    driver.find_element(*AuthPade.PASSWORD_INPUT).send_keys(random_password)
    driver.find_element(
        *AuthPade.SUBMIT_PASSWORD_INPUT).send_keys(random_password)
...
def test_registration_by_email_without_mask_get_error(driver, random_invalid_email, random_password):
    ...
    driver.find_element(*AuthPade.EMAIL_INPUT).send_keys(random_invalid_email)
    driver.find_element(*AuthPade.PASSWORD_INPUT).send_keys(random_password)
    driver.find_element(
        *AuthPade.SUBMIT_PASSWORD_INPUT).send_keys(random_password)
    driver.find_element(*AuthPade.CREATE_ACCOUNT_BTN).click()
